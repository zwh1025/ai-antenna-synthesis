...
**v3 平面阵竞赛验收实测**（`run_acceptance_v3_ai.py`，与 Taylor+LCMV 同评估器同口径）：

| 测试臂 | 73 标准方向 SLL | 200 随机方向 SLL | 精细指向 RMS |
|---|---|---|---|
| Taylor+LCMV（基线） | 73/73（-35.09） | 195/200（-34.07） | 0.028° |
| v3 直接输出 | 70/73（-34.73） | 199/200（-34.96） | 0.028° |
| v3+LCMV | 69/73（-34.63） | 194/200（-34.04） | 0.028° |

未达标方向全部在 θ=60° 扫描边缘（差 0.26–0.36 dB，基线自身裕量仅 0.09 dB）；零陷与指向与基线完全一致。AI 管线与解析管线性能等价，配合安全门控（不劣于 Taylor 则导出）后满足全部硬指标。另补 Taylor 随机 200 方向 3bw 口径数据（`run_random200_taylor_3bw.py`）：200/200 通过（-35.18 最差），修正了原始 `random_validation.json` 仅存 first-null 口径（含大扫描角主瓣截断伪影）的证据链缺口。
