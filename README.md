...
| 推理 P50 (1000轮) | 0.502 ms | 1.332 ms | 2.6x |
| 推理 P99 (1000轮) | 0.532 ms | 13.272 ms | 25.0x |
| 端到端延迟 | 0.658 ms | — | — |
| 连续吞吐量 (batch=1) | 1985/s | 367/s | 5.4x |
| 批量吞吐量 (batch=8/16) | 16284/s / 29138/s | 1809/s / 2244/s | 9.0x / 13.0x |
| 精度一致性 | max_err=7.45×10⁻⁸ | — | cos_sim≈1.0 |
| vs SOCP 23秒 | 0.504ms | — | 45,635x |

> 服务器配备 2 颗 Ascend 910_9362（各 64GB HBM），本次仅使用单卡。模型仅 0.47-1.9MB，单卡算力远超需求。vs SOCP 的加速比是"AI 前向推理"与"迭代优化求解"的算法+硬件综合差距（算法层 ≈17,000x × 硬件层 ≈2.6x），不是同类计算的硬件加速比，不能表述为"NPU 硬件单独带来 45,635 倍加速"。
